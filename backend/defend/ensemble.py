...
from backend.defend.anomaly import ChannelCResult, anomaly_scores, fit_channel_c
from backend.defend.cost import CostMatrix, cost_per_100k, matrix_from_config, optimal_threshold
from backend.defend.features import FEATURE_NAMES, FeatureContext, compute_features
from backend.defend.gbdt import (
    ChannelAResult,
    export_onnx,
    fit_channel_a,
    platt_coefficients,
...
from backend.defend.graph_channel import ChannelBResult, build_design, fit_channel_b, score_channel_b
from backend.defend.ladder import band_boundaries
...
class Detector:
    ...
    def _record_shap(self, train_features: pd.DataFrame, rng: np.random.Generator) -> None:
        from backend.defend.explain import background_sample, shap_values
    ...
    def export_onnx(self, path: Path | None = None) -> Path:
        ...
        from backend.defend.explain import reason_dictionary_payload
